...
class UpdateUser extends Component {
    constructor(props) {
        ...
        this.state = {
            kid: this.props.match.params.id,
            kpesel: '',
            kimie: '',
            knazwisko: '',
            kadrZam: '',
    ...
    componentDidMount(){
        axios.get(`http://localhost:8080/api/kierowcy/${this.state.kid}`).then((res) => {
            let user = res.data;
            this.setState({kpesel: user.kpesel,
                kimie: user.kimie,
                knazwisko : user.knazwisko,
                kadrZam : user.kadrZam,
                knrZam : user.knrZam,
                knazwMiej : user.knazwMiej
            });
        });
    }
    ...
    saveUser = (e) =>{
        ...
        axios.put(`http://localhost:8080/api/kierowcy/${this.state.kid}`, user).then((response) => {
